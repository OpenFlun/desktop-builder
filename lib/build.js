...
const __dirname = path.dirname(fileURLToPath(import.meta.url)), require = createRequire(import.meta.url),
    ...
    build = async () => {
        ...
        const windowConfig = {
            ...
            advanced = {
                autoStartServer: true, autoKillServer: true,
                ...(config.advanced || {}),
            },
        ...
        console.log(chalk.blue('[信息] 成功写入' + Object.keys(allDeps).length + ' 个依赖包到deps.json'));
        ...
        const platformInfo = {
            ...
            info = platformInfo[process.platform] || platformInfo.win32,
            currentPlatform = info.platform, platformArg = info.arg, buildConfig = config.build || {},
            // NSIS 配置
            nsisConfig = {
            ...
            // 构建配置对象,平台字段按当前系统条件添加（不再预设 null 再清理）
            configObj = {
                files: [
                    '!builder.json',
                    '!app.png',
                ...
                asar: false,
                // win / mac / linux 将根据 currentPlatform 添加
        ...
        // 根据当前平台添加对应字段
        if (currentPlatform === 'win') configObj.win = { target: ['nsis'] };
        if (currentPlatform === 'mac') configObj.mac = { target: ['dmg'] };
        if (currentPlatform === 'linux') configObj.linux = { target: ['AppImage'] };
