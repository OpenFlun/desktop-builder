...
export default async function buildApp() {
    ...
    if (!(await fs.pathExists(tempNodeModules))) {
        if (await fs.pathExists(srcNodeModules)) {
            console.log(chalk.yellow('[WARN] node_modules not found in temp dir, copying...'));
            await fs.copy(srcNodeModules, tempNodeModules, { dereference: true });
            console.log(chalk.green('[OK] node_modules copied.'));
        } else {
            console.error(chalk.red('[ERROR] No node_modules in project root. Run npm install.'));
            process.exit(1);
        }
    }
    ...
    if (iconAbsPath && (await fs.pathExists(iconAbsPath))) {
        try {
            ...
            if (width >= 256 && height >= 256) {
                ...
                console.warn(chalk.yellow('[WARN] Icon size < 256x256, using default.'));
            }
        } catch (err) {
            console.warn(chalk.yellow('[WARN] Cannot read icon: ' + err.message));
        }
    } else {
    ...
    try {
        ...
        console.error(chalk.red('[ERROR] Cannot find electron package. Install it.'));
        process.exit(1);
    }
    ...
    // 使用 extraResources 将 node_modules 复制到 app 目录下
    const configObj = {
        ...
        try {
            await execa('npx', args, {
                ...
                env: {
                    ...
                    CSC_IDENTITY_AUTO_DISCOVERY: 'false', // 禁用签名
                },
            });
            success = true;
            break;
    ...
    if (await fs.pathExists(tempDistDir)) {
        ...
        if (copiedCount > 0) {
            ...
            console.warn(chalk.yellow('[WARN] No installer found.'));
        }
    } else {
        console.warn(chalk.yellow('[WARN] No dist folder found.'));
    }
}